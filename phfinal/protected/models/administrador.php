...
class administrador extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('nick, nombre, apellido, email, pass, tipo', 'required'),
			array('tipo', 'numerical', 'integerOnly'=>true),
			array('nick, nombre, apellido', 'length', 'max'=>50),
			array('email', 'length', 'max'=>100),
			array('pass', 'length', 'max'=>150),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, nick, nombre, apellido, email, pass, tipo', 'safe', 'on'=>'search'),
		);
	}
