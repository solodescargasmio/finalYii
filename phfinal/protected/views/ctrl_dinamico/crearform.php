...
return ok;*/
    ...
    function control(){
      ...
      datatypo='nomformulario='+nomb_form;
          $.ajax({
         url: 'index.php?r=ctrl_dinamico/controlForm',//llamo a la pagina q hace el control
         type:'POST',//metodo por el cual paso el dato
         data:datatypo,
             success: function (data) { //funcion q recoge la respuesta de la pagina q hace el control
                 // $("#avizo").fadeIn(1000).html(data); //imprimo el mensaje en el div   
                 if(data!=0){  
                 alert("El formulario ya existe en el sistema.\n Puede crear una version nueva del mismo.");	
                 document.getElementById("nom_formulario").value="";
                 $('#miform').hide();
                 $('#formversion').show();
                   }
...
function capitalize(s)//convierte minusculas a Mayusculas
{
   ...
   <a href=<?php echo Yii::app()->createUrl('ctrl_dinamico/nuevaVersion')?>> <button id="ver"  class="btn btn-primary btn-group-sm">Nueva Versión</button></a> 
...
?>
    ...
    <form id="formversion" class="form-horizontal" action="" method="post" enctype="multipart/form-data">
       ...
       <tr class="version1">
    <td class="formu"><font style="font-weight: bold; color: red;"><?php echo strtoupper($formulario->nombre); ?></font></td>                 
                   </tr>
          <?php   
        ...
        <br><br>
        <?php 
    $attrRe = Yii::app()->db->createCommand("SELECT * FROM atributo WHERE nombre='relizado_por'")->queryRow();
$id_att = $estudiopa["id_attributo"];
$tipo = $estudiopa["tipo"];
      ...
      <form id="my-dynamic-form" method="POST" onsubmit="return validarFormulario()" class="form"> 
       <div class="form-group" style="border-width: 5px; background:#C8C0C0; width:56%;">
                 <label >Nombre Formulario(<font style="color: red;">*</font>)</label>
    <div class="col-lg-8">
    <input type="text" name="nom_formulario" id="nom_formulario" onblur="control();" onkeypress="return vacio(event);" required="">
     </div></div>
      </form>
        </div> 
       </div> 
